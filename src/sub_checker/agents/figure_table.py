...
class FigureTableAgent(BaseCheckerAgent):
    ...
    def _default_system_prompt(self) -> str:
        return (
            ...
            "2. Check that numbering is sequential (no gaps)\n"
            "3. Check that all figures/tables mentioned in text have corresponding "
            "legends or captions somewhere in the manuscript\n"
            "4. Check for inconsistent naming (mixing 'Figure' and 'Fig.')\n"
            "5. If figure files exist in the figures directory, check that they match "
            "the references in text\n\n"
            "IMPORTANT:\n"
            "- Missing figure IMAGE FILES is normal — manuscripts are often submitted as "
            ".docx without embedded images. Do NOT report missing image files as errors.\n"
            "- Only report missing image files as INFO if figure legends are also absent.\n"
            "- Focus on: numbering gaps, unreferenced figures/tables, missing legends, "
            "naming inconsistencies, and figures cited but never defined.\n\n"
